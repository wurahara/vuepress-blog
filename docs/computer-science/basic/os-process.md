...
1. **轮转调度** (Round Robin)
   ...
   为每个进程分配一个时间段，成为时间片 (quantum)。若时间片结束时进程尚未执行完毕，则挂起该进程，将 CPU 时间分配给另一个进程。若进程在时间片结束之前阻塞或结束，则立即切换进程。

2. **优先级调度**

   为每个进程赋予一个优先级，并允许高优先级的进程先执行。还可以将响应比作为优先级，此时该调度方法称为高响应比优先调度算法。
   $$
